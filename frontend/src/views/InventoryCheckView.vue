  ...
  <div>
    ...
    <el-card class="box-card">
      <template #header>
        <div class="clearfix">
          <!-- 库存盘点按钮 -->
          <el-button style="float: left; padding: 2px 1" plain @click="openInventoryCheckDialog">
            开始库存盘点
          </el-button>
          <span>盘点记录查询</span>
      ...
      <el-table :data="inventoryCheckRecords" stripe style="width: 100%">
        ...
        <el-table-column prop="operator_id" label="操作员ID" />
      </el-table>
...
import { getInventoryCheckRecords, getInventoryList, createInventoryCheck } from '@/api/inventoryCheck';
...
const fetchInventoryCheckRecords = async () => {
  try {
    ...
    inventoryCheckRecords.value = response.data.list;
    checkTotal.value = response.data.total || 0;
  } catch (error) {
...
const resetInventoryCheckForm = () => {
  inventoryCheckForm.operator_id = '';
  ...
  try {
    ...
    const inventoryCheckData = validItems.map((item) => {
      ...
      return {
        ...
        operator_id: parseInt(inventoryCheckForm.operator_id) || null
      };
    });

    await createInventoryCheck(inventoryCheckData);
    ElMessage.success('盘点结果提交成功');
    inventoryCheckDialogVisible.value = false; // 关闭弹窗
    refreshCheckRecords();
  } catch (error) {
